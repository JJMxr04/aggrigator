...
class League(Base, TimestampMixin):
    ...
    # Default ``active=False`` — newly-seeded leagues are OFF until an
    # operator explicitly enables them in SQLAdmin, same as Sport. Even
    # once enabled they're still gated by the parent Sport row:
    # ``ingest_due_leagues`` walks only leagues whose Sport is ALSO
    # active, so BOTH the sport and the league must be flipped on before
    # a league is walked. Re-seeding does NOT reset operator-flipped
    # league rows (per ``upsert_league_from_spec``).
